...
if addLabels:
  ...
  if plotTrain:
    ...
    for label in ttypeLabels:
      if label[4] == 'cbd':
        ax.annotate(label[1],xy=(label[2],label[3]),xytext=[2.0,-4.0],textcoords='offset pixels',color=bgColor, bbox=dict(boxstyle='square,pad=0.4', fc=colourDict['train'], ec='none'),size=1,zorder=2.0)
      else:
        ax.annotate(label[1],xy=(label[2],label[3]),xytext=[2.0,-4.0],textcoords='offset pixels',color=bgColor, bbox=dict(boxstyle='square,pad=0.4', fc=colourDict['train'], ec='none'),size=1,zorder=2.0)
  ...
  if plotTram:
    ...
    for label in ttypeLabels:
      if label[4] == 'cbd':
        ax.annotate(label[1],xy=(label[2],label[3]),xytext=[2.0,2.0],textcoords='offset pixels',color=bgColor, bbox=dict(boxstyle='square,pad=0.4', fc=colourDict['tram'], ec='none'),size=1,zorder=4.0)
      else:
  ...
  if plotBus:
    ...
    for label in ttypeLabels:
      if label[4] == 'cbd':
        ax.annotate(label[1],xy=(label[2],label[3]),xytext=[-2.0,2.0],textcoords='offset pixels',color=bgColor, bbox=dict(boxstyle='square,pad=0.4', fc=colourDict['bus'], ec='none'),size=1,ha='right',zorder=6.0)
      else:
...
if doExport:
  ...
  fig.savefig("exports/test.svg", format='svg', bbox_inches='tight')
#  printProg('test.svg done... ', False)
#  fig.savefig("exports/test.png", format='png')
  printProg("done")
  
  np.savetxt("exports/labels.csv", terminusLabels, delimiter=",", fmt='%i,%s,%f,%f,%s')
